...
public class RepositionWithSlowDown extends Command {
    ...
    public RepositionWithSlowDown(Hardware robot, ElapsedTime timer, double distance, double timeOut, double powerLevel) {
        super(robot);
        this.timer = timer;
...
//            rightEndTicks=rightFrontTicks + (int)(-(inches*HackinHoundsHardware.TICK_PER_INCH)*Math.signum(powerLevel));
            startMeasure = robot.rangeSensor.getDistance(DistanceUnit.INCH);
        }
    }
